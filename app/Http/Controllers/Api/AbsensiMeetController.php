...
use App\Models\AbsensiMeet;
use App\Models\Meeting;

class AbsensiMeetController extends Controller
{ 
        ...
        if ($request->input('dataTable') == true) {
            ...
            if($request->input('searchData')){
                $data->where(function ($query) use ($request) {
                    foreach (AbsensiMeet::getTableColumns() as $value) {
                        $query->orwhere('absensi_meeting.' . $value, 'LIKE',  "%" . $request->input('searchData') . "%");
                        $query->orwhere('absensi_meeting.' . $value, 'LIKE',  "" . $request->input('searchData') . "%");
                        $query->orwhere('absensi_meeting.' . $value, 'LIKE',  "%" . $request->input('searchData') . "");
                        $query->orwhere('absensi_meeting.' . $value, 'LIKE',  $request->input('searchData'));
                    }
    ...
    {  
        ...
        foreach (AbsensiMeet::getTableColumns() as $key) {  
        ...
        $dummy = AbsensiMeet::create($result);
        if ($dummy) {
    ...
    public function scanQr(Request $request)
    {
        ...
        $data = Meeting::where('token', $token)->first();
        if (isset($data)) { 
            $time = time();
            ...
            // if ($time < $date) {   
                ...
                if($cekAbsen){ 
                    ...
                    if($data->status_kelas == 0 && $stPosisi == false){       // Jika Kelas Offline dan Tidak Masuk radius 
                        ...
                        $dummyAbsen = AbsensiMeet::create($absensi);
                        if ($dummyAbsen) {
